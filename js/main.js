...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const energyElement = document.querySelector('.js-energy');
    const progressElement = document.querySelector('.user-info__rank-progress span');
    ...
    tapButton.addEventListener('click', () => {
        ...
        let currentProgress = parseFloat(progressElement.style.getPropertyValue('--progress'));
        
        currentProgress -= 1;
        if (currentProgress < 0) currentProgress = 0;
        progressElement.style.setProperty('--progress', `${currentProgress}%`);
    });
});